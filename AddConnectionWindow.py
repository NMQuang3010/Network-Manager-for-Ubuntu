...
class AddConnectionWindow(Gtk.Window):
    def get_if_network_by_connection_type(self,device_type):
        ...
        try:
            ...
            for if_device in if_devices:
                ...
                if_hwaddr=result.stdout.strip().split('\n')
                if_network=if_device+"("+if_hwaddr[0]+")"
    ...
    def __init__(self,connection_type,network_connection_window):
        Gtk.Window.__init__(self, title="Add Connection")
    
        self.set_default_size(600, 400)
        self.set_border_width(10)
        self.network_connection_window = network_connection_window
        self.connection_type=f"{connection_type.lower()}"
        ...
        self.mac_combo = Gtk.ComboBox.new_with_model(self.list_mac)
        self.mac_combo.set_entry_text_column(0)  # Use index 0 for text column
        self.renderer_text = Gtk.CellRendererText()
        self.mac_combo.pack_start(self.renderer_text, True)
        self.mac_combo.add_attribute(self.renderer_text, "text", 0)
    ...
    def is_valid_format_v4(self, text,column):
        ...
        #kiểm tra chiều dài submnet mask
        if column_id == "Netmask                     " and text and (0 <= int(text) <= 32):
           return True
        #  kiểm tra định dạng chuỗi (ví dụ: "122.333.222.111")
        try:
            parts = text.split('.')
    ...
    def add_connection(self,connection_name,connection_type, if_name,if_hardware ,ipv4_method , ipv4_addresses,ipv4_gateway,ipv4_dns,ipv4_dns_search,
                       ipv6_method,ipv6_addresses,ipv6_gateway,ipv6_dns,ipv6_dns_search):
      # Chuỗi lệnh để thêm kết nối Ethernet
      command_line = f"nmcli connection add con-name '{connection_name}' type {connection_type} ifname {if_name}   \
      ...
      # Thêm địa chỉ IPv4 tĩnh cho kết nối
      ...
      # Thêm địa chỉ IPv6 tĩnh cho kết nối
      ...
      try:
        ...
        self.show_success_dialog(result.stdout)
      except subprocess.CalledProcessError as e:
        print(f"Error: {e}")
    ...
    def on_button_save_clicked(self,widget):
        if self.is_valid_form_save():
           ...
           switch_active_v4 = self.switch.get_active()
           switch_active_v6 = self.switchv6.get_active()
           #ipv4
           ...
           if not switch_active_v6:
               ...
               ipv6_method="auto"
           self.add_connection(name,self.connection_type,interface_name,interface_hardware,ipv4_method,ipv4_list,gw4_list,ipv4_dns,ipv4_dns_search,
